...
import { Loader2, Zap, Shield, Sparkles, Gamepad2, Check, TicketPercent, Lock, X } from "lucide-react"; // ✅ เพิ่มไอคอน X
...
export default function GameShopClient({ game }: { game: Game }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedProduct) return alert("กรุณาเลือกแพ็กเกจก่อนครับ");
    if (!isOpen) return alert("ขออภัย ร้านปิดให้บริการชั่วคราวครับ");
      ...
      <div className="relative z-10 w-full max-w-6xl space-y-8 pb-32 md:pb-0"> 
        ...
        <AnimatePresence>
          {selectedProduct && isOpen && ( 
            ...
            >
              {/* ✅ ปรับปรุงโครงสร้างให้รองรับ Scroll ในมือถือ และมีปุ่มปิด */}
              <div className="w-full bg-[#0f0f13]/95 backdrop-blur-xl border border-white/10 rounded-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.5)] relative overflow-hidden">
                ...
                {/* ✅ ปุ่ม X สำหรับปิดฟอร์ม (แสดงชัดเจนในมือถือ) */}
                <button
                    onClick={() => setSelectedProduct(null)}
                    className="absolute top-4 right-4 z-30 p-2 rounded-full bg-white/5 hover:bg-red-500/20 text-slate-400 hover:text-red-400 transition-all md:hidden"
                >
                    <X className="w-6 h-6" />
                </button>

                {/* ✅ โซนเนื้อหาที่สามารถใช้นิ้วเลื่อนขึ้นลงได้ (max-h) */}
                <div className="p-6 md:p-8 max-h-[80vh] overflow-y-auto">
                    <div className="flex flex-col md:flex-row gap-8">
                        {/* Left: Summary */}
                        <div className="flex flex-row md:flex-col items-center md:items-start gap-4 md:w-1/3 border-b md:border-b-0 md:border-r border-white/10 pb-6 md:pb-0 md:pr-8 pr-10">
